...
@pytest.mark.asyncio
async def test_workflow_runtime_default_main_path_smoke(tmp_path: Path) -> None:
...
'''
    ...
    task_completed = next(event for event in events if event.type == "task_completed")
    task_id = task_started.data["task_id"]
    assert task_started.data == {"task_id": task_id, "subagent_type": "general-purpose"}
    assert task_completed.data["task_id"] == task_id
    assert task_completed.data["result_preview"] == "Subagent result"
    assert events[-1].data["subagent_runs"][task_id]["status"] == "completed"
